...
function idSet<T>(items: Iterable<T> | undefined) {
  return items ? new Set(items) : new Set<T>();
}

export type SubmitTrackRow = {
  _id: Id<"tracks">;
  slug: string;
  label: string;
  note: string;
...
export type SubmitMine = FunctionReturnType<typeof api.submissions.mine>;
export type SubmitCatalog =
  | FunctionReturnType<typeof api.perks.listCatalog>
  | undefined;
...
export function SubmitFlow({
  ...
  empty?: ReactNode;
  mine: SubmitMine;
  onSubmit: (args: SubmitProjectArgs) => Promise<unknown>;
  submissionsOpen: boolean;
  teamRepo: string;
  tracks: SubmitTrackRow[];
}) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const [lastSubmitted, setLastSubmitted] = useState<string | null>(null);
  const submittedIds = idSet(mine?.submittedTracks?.map((row) => row._id));
  const registered = idSet(mine?.challengeIds);
  const pending = tracks.filter((track) => {
    if (submittedIds.has(track._id)) {
      return false;
    }
    if (registered.size === 0) {
      return allowAnyTrack;
    }
    return registered.has(track._id);
  });
  ...
  if (pending.length === 0) {
    if ((mine?.submittedTracks ?? []).length > 0) {
      return <DoneCard tracks={mine?.submittedTracks ?? []} />;
    }
    return (
      empty ?? (
        <Card>
