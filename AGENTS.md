...
- `hackspain profile` mirrors `src/app/profile/page.tsx` one to one (`users.setAttendance`, `users.updateEventDetails`, `users.setNotificationConsent`, `onboarding.requestPhoneCode/verifyPhoneCode`, `github.startLink/unlink`); it adds no new backend functions, only allowlist entries. GitHub linking is the same OAuth flow: the CLI prints the authorise URL and the callback lands on the dashboard.
