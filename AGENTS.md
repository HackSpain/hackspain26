...
- `/tv`, `/final/cancelar`, the reception path, and the closing path skip login. Keep them in `isPublicAppPath` and the middleware public matcher together; the cancel page authenticates with the email token, not a session.
