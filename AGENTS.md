...
- `hackspain watch` collects AI-harness usage into the canonical `hackspain.telemetry.v2` event (`apps/cli/src/watcher/schema.ts`, documented in `apps/cli/docs/telemetry-schema.md`). Collectors live in `src/watcher/collectors/` (Claude Code, Codex, Gemini CLI, Qwen Code, OpenCode, Kilo Code as an OpenCode fork sharing `opencode.ts`, Cline) and must fail soft. Events go to a local NDJSON spool; the exact HTTP batch is persisted before upload and retried across restarts. `POST /api/cli/telemetry` (`apps/app/src/app/api/cli/telemetry/route.ts`) authenticates, validates, and exports the accepted events as OTLP/JSON through RawTree's native logs endpoint. RawTree needs `RAWTREE_API_KEY` with `write_only` permission and `RAWTREE_DATABASE` in the dashboard environment; logs use the `logs` table by default and `RAWTREE_OTLP_LOGS_TABLE` can override it. The `/insights` page and the TV's insight and live boxes (`src/components/tv/insights-boxes.tsx`, `live-boxes.tsx`) read that table back: `GET /api/tv/insights` (public like `/api/tv`, cached 30 s in the function) runs one aggregate in RawTree (`usageSql` in `api/tv/insights/usage.ts`: dedupes on (`hackspain.user.id`, `event.id`), buckets the hackathon window into `INSIGHT_BUCKETS` on OTLP `timeUnixNano`, counts a session only in its first bucket) with `RAWTREE_READ_API_KEY` (a `read_only` key; the ingestion key is `write_only`), and adds teams, the window and GitHub pushes and pull requests per bucket from the public Convex query `tvPlayback.insightsBase`. `src/app/insights/use-live-insights.ts` is the single poller per page and maps the payload to the `Sample`/`Team` shapes the charts take; usage from people without a team counts in totals under `NO_TEAM_ID` and never in a ranking. The helpers in `insights/mock-data.ts` keep their static defaults (`getSamples()` is empty, a 12-hour day of 30-minute buckets) and take the real teams and a `Timeline` as optional arguments: with one, labels are real Madrid time and the period filters end at the bucket the clock is in (`periodBuckets`). GitHub activity has no harness, so nothing may show pushes per tool. The evolution tab's phases, overlapping sessions, milestones and cost are synthetic (`event-data.ts`) and only render on the static layout; on real data `RealEvolution` shows what the telemetry supports. Organiser broadcasts are polled through `/api/cli/rpc` (`notifications:forMe`) from the last `sentAt` seen by any run. `watcher/memory.ts` persists `watch-memory.json` (first start, last scan, last announcements): on start the board replays the spool (`replaySpool`) so totals and recent requests cover everything since the first run, announcements are restored, and logs written while closed are collected because `since` is the start of the hackathon, not the last run. `watcher/window.ts` is the rule, the same for every account, organisers included: only the scheduled hackathon window is recorded, whole and on `occurredAt` (the harness's time, never the read time), and nothing at all without a schedule. The cursor store starts over when `since` moves earlier than its `coveredSince`, the window is re-read every five minutes, the watcher also runs before, after and without a schedule with a "Not recording" notice from `windowNotice` (and without polling team, announcements or feed when the server has them closed), and the telemetry route rejects every event outside the window (`outside_event_window`, `occurredInWindow` in `telemetry/rawtree.ts`), so RawTree never holds one. Toasts only fire for announcements under ten minutes old. Fixtures under `apps/cli/test/fixtures` are redacted; a test rejects home paths.
- Telemetry is one schema with one meaning per field, `hackspain.telemetry.v2`. Collectors yield only what the harness logged (`RawEvent`); every derived field (`model.name`, `model.family`, `model.provider`, `tokens.total`) comes from the pure module `apps/app/src/app/api/cli/telemetry/canonical.ts`, which the CLI bundles by relative path (`canonicalize` in `watcher/schema.ts`, called by `stamp`) and which `parseTelemetryEvent` runs again on ingestion, ignoring the client's values. v1 events from binaries up to 0.4.x are accepted and stored as v2; `upgradeEvent` does the same for old spool lines. `tokens.input` excludes cache reads and `tokens.output` includes reasoning for every harness (`outputWithReasoning`). Anything only some harnesses report goes under `native` (`requestId`, `costUsd`) and is never compared across harnesses. Never derive a field inside a collector.
- Telemetry update checklist: when adding or changing collected data, update `apps/cli/src/watcher/schema.ts`, the matching type and validator in `apps/app/src/app/api/cli/telemetry/rawtree.ts`, `apps/cli/docs/telemetry-schema.md`, the relevant collector/ingestion tests, and every RawTree query or dashboard that consumes it. A new optional field can use RawTree's Dynamic schema without a migration. Renaming a field, changing its type, or changing its meaning is breaking: bump `SCHEMA`, document the compatibility window, and keep ingestion compatible with the previous version during rollout. Keep `native` small, add every permitted key to both validators, and never place prompts, responses, code, full paths, environment variables, credentials, or harness account ids in telemetry. Native OTLP does not guarantee insert deduplication: consumers must deduplicate permanently by (`hackspain.user.id`, `event.id`) before aggregating.
