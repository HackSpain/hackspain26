...
| `/onboarding` | Accepted hacker confirms phone, terms and notification consent |
| `/` | Home: section tiles plus the feed (composer + timeline with author, team and project context) |
| `/participantes` | Connections graph on real data (`directory.list`); gated behind the viewer's own card (`directory.me` / `save`) |
