...
- Project submit is dashboard `/submit` (YouTube + public GitHub + optional product URL), not the CLI. One team, one track. The home Submit tile features from Sunday 08:00 Europe/Madrid and the form locks at 11:05 (`apps/app/src/lib/event.ts`). CLI `saveDraft` still stores a draft; `hackspain submit` is gone.
